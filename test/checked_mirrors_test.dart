...
main() {
  ...
  group('get name,', () {
    ...
    test('symbol is not declared', () {
      expect(MirrorSystem.getName(#x), 'x');
      expect(records.length, 1); // woo hoo! this was detected
      var r = records.removeLast();
      expect(r.kind, MirrorsUsedWarningKind.UNDECLARED_SYMBOL);
      expect(r.symbol, #x);

      expect(MirrorSystem.getSymbol('x'), #x);
  ...
  group('get field,', () {
    ...
    test('symbol is not declared', () {
      expect(reflect(a).getField(#x).reflectee, 1);
      expect(records.length, 1);
      var r = records.removeLast();
      expect(r.kind, MirrorsUsedWarningKind.TARGET_NOT_MARKED);
      expect(r.object.reflectee, a);
      expect(r.symbol, #x);
    });

    test('even if symbol is declared', () {
      expect(reflect(a).getField(#y).reflectee, 2);
      expect(records.length, 1);
      var r = records.removeLast();
      expect(r.kind, MirrorsUsedWarningKind.TARGET_NOT_MARKED);
      expect(r.object.reflectee, a);
    ...
    test('meta target is declared on field', () {
      ...
      expect(reflect(d).getField(#z).reflectee, 7);
      expect(records, hasLength(1));
      var r = records.removeLast();
      expect(r.kind, MirrorsUsedWarningKind.TARGET_NOT_MARKED);
      expect(r.object.reflectee, d);
      expect(r.symbol, #z);
    });
  });
